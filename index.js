...
function extractTextInParentheses(str) {
  ...
  const arrText = Text.map(match => match[1]);
  return arrText
...
function countEmails(str) {
  ...
  const emails = [...str.matchAll(regex)];
...
function replaceWordOccurrences(str, word, newWord) {
  ...
  const replacedWord = str.replace(regex, newWord);
  return replacedWord;
...
function checkRegexMethods(regex) {
  let useMethod = [];
   if (regex.flags.includes('s')) {
    useMethod.push(`dotAll`) 
   };
   if (regex.multiline) {
    useMethod.push('multiline');
  };
  if (regex.sticky) {
    useMethod.push('sticky');
  }
     return useMethod
